...
asyncTest("datalist", function(){
	...
	if(!Modernizr.datalist){
		strictEqual($('#email').attr('aria-haspopup'), undefined, 'aria was removed on input:not([list])');
	}
	...
	if(!Modernizr.datalist){
		...
		strictEqual($('#email').attr('aria-haspopup'), undefined, 'removed list attribute removes aria in shim');
